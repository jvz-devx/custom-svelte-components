// Balatro foil.fs — exact port to WebGL2
// Computes foil vec2 from time+mouse, uses Balatro UV mapping via texture_details/image_details
...
void main()
{
    ...
    // foil vec2 from Balatro: [send_to_shader[1], send_to_shader[2]]
    vec2 foil = edition_params;
    ...
    gl_FragColor = dissolve_mask(tex, vUv, uv);
}
