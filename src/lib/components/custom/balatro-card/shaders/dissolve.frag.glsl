...
// Dissolve fragment shader — ported from Balatro dissolve.fs
// Primary shader for ALL card rendering. Identity pass when dissolve < 0.001.
...
void main() {
    vec4 tex = texture(cardTexture, vUv);
    vec2 uv = vUv;
    ...
    fragColor = dissolve_mask(tex, vUv, uv);
}
